$('input[type="number"]').bind('input', function () { 
    ...
    var filetaniguchi = document.getElementsByName('am');
    for(let i = 0; i < filetaniguchi.length; i++){
        var a = Number(filetaniguchi[i].value);
        All += 20000*a;
    }

    var fileterao = document.getElementsByName('bp');
    for(let i = 0; i < fileterao.length; i++){
        var a = Number(fileterao[i].value);
        All += 30000*a;
    }

    var filehiramoto = document.getElementsByName('dss');
    for(let i = 0; i < filehiramoto.length; i++){
        var a = Number(filehiramoto[i].value);
        All += 15000*a;
    }

    var filefukuda = document.getElementsByName('lps');
    for(let i = 0; i < filefukuda.length; i++){
        var a = Number(filefukuda[i].value);
        All += 18000*a;
    }

    var charmokubo = document.getElementsByName('iphs');
    for(let i = 0; i < charmokubo.length; i++){
        var a = Number(charmokubo[i].value);
        All += 15000*a;
    }

    var card = Math.floor(All / 60000)

    document.getElementById("result-erea").innerHTML = "<h3 class='text-white'>合計" + Allnum + "点</h3>" + "<h3 class='text-white'>KRW" + All + "</h3><h3 class='text-white'>トレカ" + card + "枚</h3>";
    document.getElementById("result").innerHTML = "<h3 class='text-primary'>合計" + Allnum + "点</h3>" + "<h3 class='text-primary'>KRW" + All + "</h3><h3 class='text-primary'>トレカ" + card + "枚</h3>";
});
